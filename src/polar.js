...
const getPolarDataForDate = async (date) => {
...
exports.postToPolar = async (event) => {
  console.log(event)
  await login()
  const currentData = await getPolarDataForDate(event.date)
  await postNewWeight(currentData, event.weight)
  console.log("Done!")
}
